...
void stampa(){
    ...
    for(ll i=0; i<RR; i++){
        ...
        if(p[z].r == i && z < (int)p.size()) cout << "   ";
        while(p[z].r == i && z < (int)p.size()){ cout << p[z].tipo << "(" << p[z].vita << ")"; if(++z<(int)p.size() && p[z].r == i ) cout << ", "; }
        cout << "\n";
    }
    cout << "\n";
}
...
ll indexPlayerAt(ll r, ll c){
    for(ll i=0; i<(ll)p.size(); i++) if(p[i].r == r && p[i].c == c) return i;
    return 0;
}
